      ...
      try {
        const context = {
          ...
          log: {
            ...
            error: (msg: string) => this.#logger.error(`[tool:${name}:error] ${msg}`),
            info: (msg: string) => log(`[tool:${name}:info] ${msg}`),
            warn: (msg: string) => this.#logger.warn(`[tool:${name}:warn] ${msg}`),
          },
        ...
        const result = await tool.execute(args as Parameters<typeof tool.execute>[0], context);
    ...
    while (iterations < maxIterations) {
      ...
      for (const tc of assistantMsg.tool_calls) {
        ...
        try {
          ...
          log(`[testWithOllama] Malformed JSON in tool args: ${tc.function.arguments}`);
        }

        log(`[testWithOllama] Calling tool: ${tc.function.name}(${JSON.stringify(args)})`);
